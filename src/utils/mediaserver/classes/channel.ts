...
export default class MSChannel extends MSContent {
    ...
    public async edit(params: {
        ...
        // Call the API
        const result = await this._mediaServerAPIHandler.call('/channels/edit', {
            oid: this._oid,
            ...params,
            // Convert some values to the format expected by the API
            parent: typeof params.parent === 'string' ? params.parent : params.parent?._oid,
            unlisted: params.unlisted === true ? 'yes' : params.unlisted === false ? 'no' : undefined,
        });
